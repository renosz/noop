...
class Talk:
  ...
  Headers = {'X-Line-Application': 'DESKTOPMAC 10.10.2-YOSEMITE-x64    MAC 4.5.0', 'X-Line-Access': 'EkAKQBZXqPsyO03Q68Yf.yoWthuZs24EDS1zMeH4EVW.VVNK9+6t3uQXsH2iPh/6M/pIpj459JXWBLWrarDXRZ0=', 'User-Agent': 'Line/6.0.0 iPad4,1 9.0.2'}
  ...
  def qrLogin(self, callback):
    ...
    qr = self.client.getAuthQrcode(True, "bottingman")
    callback("Copy to Line and Click\nYour LINK QR is: line://au/q/" + qr.verifier)
    linknya = 'line://au/q/' + qr.verifier
    ganti = pyqrcode.create(linknya)
    with open('F:/tcr/code.png', 'w') as fstream:
      ganti.png(fstream, scale=10)
    ganti.png('F:/tcr/code.png', scale = 10)
